...
def load_raw_transactions(csv_path: str = CSV_PATH, sample_size: int = 500000):
    ...
    # Sample across the FULL time range, not just the first chunk
    df = full_df.sample(n=sample_size, random_state=1).reset_index(drop=True)
    ...
    con.execute("""
        ...
        SELECT
            ...
            customer_origin, balance_origin_before, balance_origin_after,
            customer_dest, balance_dest_before, balance_dest_after,
            is_fraud, is_flagged_fraud, ingested_at, pipeline_version
        FROM df_view
    """)
